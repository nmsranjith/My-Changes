    ...
    function AddSelectedTeacher()
	{
		...
		$('.gchkbx span').each(function(){
			if($(this).attr('class').trim()=='ico-check')
			{
				...
				$("<li id='S" + studID + "' class=\'SelectedItem SelectedTeacher\'><span title=" + teacherName + ">" + (teacherName.length > 10 ? teacherName.substring(0, 9) + ' ...' : teacherName) + "</span><span style='display:none'>" +studID+ "</span><a onclick='Remove(this)'>x</a></li>").appendTo("#SelectedStudentList");
	...
	jQuery("#groupssortdpn").kendoDropDownList({
        ...
        if (searchAutoComplete == undefined) {
            $("#SearchTextBox").kendoAutoComplete({
                dataSource: {
                    transport: {
                        read: {
                            url: GetFile('/DesktopModules/eCollection_Groups/GroupsHandler.ashx?Search=teacherAutoComplete'),
                            type: "GET",
        ...
        totalCheckbx = $("#TeacherRep #TeacherRepeaterDiv #TeacherRepeaterContentDiv input[type=checkbox]").length;
        checkedCount = $('#TeacherRep #TeacherRepeaterDiv #TeacherRepeaterContentDiv input:checked').length;
        CheckAllCheck();
        jQuery('#eCollectionContent').height((jQuery('#StudentDiv').height() + jQuery('.GroupAddto').height() + 80) + 'px');
        ...
        if ($("#SelectedStudentList li").length >= 1) {
            for (var j = 0; j < $("#SelectedStudentList")[0].childNodes.length; j++) {
                ...
                if (tis.nodeName != "#text") {
                    var RefNode = $("#TeacherRep #TeacherRepeaterDiv #TeacherRepeaterContentDiv input[type=checkbox]");
    ...
    function CheckAll(obj) {
        if (!checkallFlag) {
            checkedCount = $("#TeacherRep #TeacherRepeaterDiv #TeacherRepeaterContentDiv input[type=checkbox]").length;
            var RefNode = $("#TeacherRep #TeacherRepeaterDiv #TeacherRepeaterContentDiv input[type=checkbox]");
            for (var i = 0; i < RefNode.length; i++) {
                ...
                if (!RefNode.parent()[i].children[0].checked) {
                    ...
                    $("<li id='S" + check + "' class=\'SelectedItem SelectedTeacher\'><span title=" + teacherName + ">" + (teacherName.length > 10 ? teacherName.substring(0, 9) + ' ...' : teacherName) + "</span><span style='display:none'>" + $('#' + check).parent().children()[1].innerHTML.trim() + "</span><a onclick='Remove(this)'>x</a></li>").appendTo("#SelectedStudentList");
    ...
    function CheckAllCheck() {
        var totalCheckbx = $("#TeacherRep #TeacherRepeaterDiv #TeacherRepeaterContentDiv input[type=checkbox]").length;
    ...
    function SetContentHeight() {
        ...
        jQuery('#eCollectionContent').height((jQuery('#StudentDiv').height() + jQuery('.GroupAddto').height() + 25) + 'px');
        jQuery('#eCollectionMenu').height((jQuery('#eCollectionContent').height() - 9) + 'px');
    }
    function Check(chk, unchk) {
        if ($($('#' + chk).parent().parent().children()[2].children[0]).val() == "ADD") {
            ...
            $("<li id='S" + chk + "' class=\'SelectedItem SelectedTeacher\'><span title=" + teacherName + ">" + (teacherName.length > 10 ? teacherName.substring(0, 9) + ' ...' : teacherName) + "</span><span style='display:none'>" + $('#' + chk).parent().children()[1].innerHTML.trim() + "</span><a onclick='Remove(this)'>x</a></li>").appendTo("#SelectedStudentList");
