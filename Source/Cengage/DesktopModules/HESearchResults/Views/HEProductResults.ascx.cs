...
{
    // --------------------------------------------------------------------------------------------------------------------
    // <copyright file="ProductResults" company="CENGAGE LEARNING AUSTRALIA PTY LIMITED">
    //    Copyright (c) 2014 CENGAGE LEARNING AUSTRALIA PTY LIMITED
    // </copyright>
    // <summary>
    //    Provides the products result page.
    // </summary>
    // ------------------------------------------------------------------------------------------------------------------------------
    public partial class HEProductResults : HESearchResultsModuleBase
    {
        ...
        List<Products> searResults = new List<Products>();
        ...
        private void FillProductResults(string q, string division, int pNo, int cpNo)
        {
            ...
            try
            {
                ...
                string queryKeyword = string.Empty, s = Request.QueryString["s"], dt = Request.QueryString["dt"], dc = Request.QueryString["dc"],
                           ...
                           sf = Request.QueryString["sf"], dCheck = Request.QueryString["dcheck"];
                ...
                string div = (division.ToLower() == "higher" ? "Higher Education" : "Vocational");

                if (UserDetailInfo.CountryCode.ToUpper() == "NZ")
                    ((DotNetNuke.Framework.CDefault)this.Page).Title = "Search" + " " + q + " " + "Page" + " " + (pNo == 0 ? 1 : pNo) + " | " + div + " | Cengage Learning - New Zealand";
                else
                    ((DotNetNuke.Framework.CDefault)this.Page).Title = "Search" + " " + q + " " + "Page" + " " + (pNo == 0 ? 1 : pNo) + " | " + div + " | Cengage Learning - Australia";
                ...
                sParams = new SearchParameters()
                {
                    ...
                    GstFlag=Null.SetNullString(UserDetailInfo.GstApplicable)
                };
                // }
                ...
                if (!string.IsNullOrEmpty(st))
                {
                    ...
                    switch (st.ToLower())
                    {
                        case "ad":
                            ...
                            string[] advFacetArray = { fv, pv, pyv, ov };
                            sParams.AdvFacets = GetAdvFacetTable(advFacetArray);
                            searchResult = HESearchResultController.Instance.GetAdvanceSearchProductResults(sParams);
                            break;
                        case "advisbn":
                            ...
                            searchResult = HESearchResultController.Instance.GetHEAdvanceIsbnResults(sParams);
                            break;
                        default:
                            break;
                    }
                ...
                else
                {
                    ...
                    searchResult = HESearchResultController.Instance.GetProductResults(sParams);
                }
                ...
                if (searchResult != null && searchResult.HasRows)
                {
                    ...
                    while (searchResult.Read())
                    {
                        ...
                        if (UserDetailInfo.TradingAccountSK > 6)
                        {
                            ...
                            productList.Add(new Products()
                            {
                                ...
                                IMAGE_FILE_NAME = string.Concat(imageUrl, "HER", Null.SetNullString(searchResult["IMAGE_FILE_NAME"])),
                                ...
                                AUDIENCE_TARGET = Null.SetNullString(searchResult["AUDIENCE_TARGET"]),
                                NEW_EDITION = Null.SetNullString(searchResult["NEW_EDITION"]),
                                EDITION = Null.SetNullString(searchResult["EDITION"]),
                                SUPPLEMENTS = Null.SetNullString(searchResult["SUPPLEMENTS"]),
                                CODE = Null.SetNullString(searchResult["CODE"]),
                                CODE_NAME = Null.SetNullString(searchResult["CODE_NAME"]),
                                PRODUCT_FORMAT = Null.SetNullString(searchResult["PRODUCT_FORMAT"]),
                                DetailUrl = CengageSearchResult.getHEProductURL(Null.SetNullString(searchResult["ISBN_13"]), Null.SetNullString(searchResult["TITLE"]), Request.QueryString["division"]),
                        ...
                        else
                        {
                            ...
                            productList.Add(new Products()
                            {
                                ...
                                CODE_NAME = Null.SetNullString(searchResult["CODE_NAME"]),
                                PRODUCT_FORMAT = Null.SetNullString(searchResult["PRODUCT_FORMAT"]),
                                FAVOURITE_FLAG = Null.SetNullString(searchResult["FAVOURITE_FLAG"]) == string.Empty ? "N" : Null.SetNullString(searchResult["FAVOURITE_FLAG"]),
                                DetailUrl = CengageSearchResult.getHEProductURL(Null.SetNullString(searchResult["ISBN_13"]), Null.SetNullString(searchResult["TITLE"]), Request.QueryString["division"]),
                                PriceVal = "",
                                DiscountRate = -1,
                                DiscountVal = -1,
                        ...
                        ProductPrice.ISBN = Null.SetNullString(searchResult["ISBN_13"]);
                        ...
                        ProductAvail.ISBN = Null.SetNullString(searchResult["ISBN_13"]);
                    ...
                    else
                    {
                        if (UserDetailInfo.TradingAccountSK > 6)
                        {
                            Priceservice(productList, ProductPriceList, ProductAvailable);
                    ...
                    if (searchResult.HasRows)
                    {
                        ...
                        while (searchResult.Read())
                        {
                            ...
                            facets.ATTRIBUTE_NAME = Null.SetNullString(searchResult["ATTRIBUTE_TYPE"]);
                            facets.ATTRIBUTE_TYPE_VALUE = Null.SetNullString(searchResult["ATTRIBUTE_TYPE_VALUE"]);
                            ...
                            ftss = string.Concat(ftss, "ATTRIBUTE_TYPE_SK-->", facets.ATTRIBUTE_TYPE_SK, "  ATTRIBUTE_NAME-->", facets.ATTRIBUTE_NAME, "  ATTRIBUTE_TYPE_VALUE_SK-->", facets.ATTRIBUTE_TYPE_VALUE_SK, "  ATTRIBUTE_TYPE_VALUE_SK-->", facets.ATTRIBUTE_TYPE_VALUE_SK, "  PROD_COUNT-->", facets.PROD_COUNT, "  IS_SELECTED-->", facets.IS_SELECTED, "\r\n");
                    ...
                    if (searchResult.HasRows)
                    {
                        ...
                        while (searchResult.Read())
                        {
                            ...
                            ftss = string.Concat(ftss, "ATTRIBUTE_TYPE_SK-->", facets.ATTRIBUTE_TYPE_SK, "  ATTRIBUTE_NAME-->", facets.ATTRIBUTE_NAME, "  PROD_COUNT-->", facets.PROD_COUNT, "\r\n");
                            lst.Add(facets);
                        }
                        LogValues(ftss);
                    ...
                    //Retrieve all subjects of the respective disciplines for the current search
                    searchResult.NextResult();
                    if (searchResult.HasRows)
                    {
                        ...
                        while (searchResult.Read())
                        {
                            ...
                            disAttr.Subject_Sk = Null.SetNullInteger(searchResult["SUBJECT_SK"]);
                            disAttr.Subject_Name = Null.SetNullString(searchResult["SUBJECT_NAME"]);
                            disAttr.Products_Count = Null.SetNullInteger(searchResult["SUBCOUT"]);
                            var attrChk = disAttr.Discipline_Category_Name_Sk + "|" + disAttr.Discipline_Sk;
                            if (!disciplineAttr.ContainsKey(attrChk))
                            {
                                lst = new List<DisciplineCategorySubjects>();
                                disciplineAttr.Add(attrChk, lst);
                            }
                            lst.Add(disAttr);
                            Attrbutetype_sk = attrChk;
                    ...
                    {
                        disCategoryAttr = Session["HEHierarchyChild1" + HttpContext.Current.Session.SessionID] as Dictionary<string, List<DisciplineCategorySubjects>>;
                        disciplineAttr = Session["HEHierarchyChild2" + HttpContext.Current.Session.SessionID] as Dictionary<string, List<DisciplineCategorySubjects>>;
                        lstDiscpline = Session["HEHierarchy" + HttpContext.Current.Session.SessionID] as List<DisciplineCategorySubjects>;
                    ...
                    {
                        Session["HEHierarchyChild1" + HttpContext.Current.Session.SessionID] = disCategoryAttr;
                        Session["HEHierarchyChild2" + HttpContext.Current.Session.SessionID] = disciplineAttr;
                        searchResult.NextResult();
                        ...
                        Session["HEHierarchy" + HttpContext.Current.Session.SessionID] = lstDiscpline;
                        DisciplineCategoryRptr.DataSource = lstDiscpline;
                        DisciplineCategoryRptr.DataBind();
                    }
        ...
        private void Priceservice(List<Products> searResults, List<Product> ProductPriceList, List<Product> ProductAvailableList)
        {
            bool Isbmup = false;
            ...
            try
            {
                ...
                searResults.ForEach(u =>
                {
                    ...
                    if (ProdPrice.UnitPriceActual < ProdPrice.RRP)
                    {
                        ...
                        u.DiscountRate = ProdPrice.DiscountrateActual;
                        u.DiscountVal = ProdPrice.DiscountedUnitPrice;
                    }
                    else
                    {
                        u.PriceVal = "$" + String.Format("{0:#,##0.00}", Convert.ToDecimal(ProdPrice.RRP.ToString()));
                    ...
                    DnnLog.Info("BM Allow sale for " + u.ISBN_13 + " : " + u.AllowSale);
                ...
                ProductResultsRptr.DataSource = searResults.Where(x => x.AllowSale == "Y");
                ProductResultsRptr.DataBind();
            }
            catch (Exception ex)
            {
        ...
        protected string BindHtml(string type, string value, string parentValue)
        {
            ...
            switch (type)
            {
                case "AUDIENCE_TARGET":
                    return "<div class=\"country\"><span class=\"AvailabilityCheck\">" + value + "</span></div>";
                    break;
                case "NEW_EDITION":
                ...
                case "PREFERRED_NAME":
                    ...
                    break;
                case "SUPPLEMENTS":
                    return "<div class=\"supplements\"><span class=\"ico-supplements\"></span><strong>Supplements</strong> <span class=\"SuppAvl\">" + value + "</span></div>";
                    break;
                case "CODE":
                    return "<div class=\"supplements\"><span class=\"ico-badge\"></span><strong>" + parentValue + "</strong><span class=\"AvailabilityCheck\">" + value + "</span></div>";
                    break;
                case "PRINT_PRICE":
                    ...
                    break;
                case "EBOOK":
                    return "<div class=\"span3 padding-10\"><div class=\"txt\"><span class=\"eBookCheck\">" + parentValue + "</span></div><div class=\"amt\">$<span class=\"AvailabilityCheck1\">" + value + "</span></div></div>";
                    break;
                case "ECHAPTER":
                    return "<div class=\"span3 padding-10\"><div class=\"txt\">eChapters</div><div class=\"amt\"><span class=\"eChapterFlag\">" + value + "</span></div></div>";
                    break;
                default:
                    break;
            }
        ...
        /// <returns></returns>
        [WebMethod]
        protected string BindLoggedInHtml(string type, string value, string parentValue, string Isbn,string allowsale)
        {
            string html = string.Empty;
            //if (visitor.UserID != 0)
            //{
            switch (type)
            {
                ...
                case "PriceVal":
                    return "<div class=\"span3 padding-5\"><div class=\"txt\">" + parentValue + "</div><div class=\"amt\"><span class=\"AvailabilityChecklogin\">" + value +
                        "</span></div></div>";
                    break;
                case "DiscountRate":
                    return "<div class=\"span3 padding-10\"><div class=\"txt\"><span class=\"eBookCheck\">" + parentValue + "</span></div><div class=\"amt\"><span class=\"AvailabilityChecklogin\">" + value + "%</span></div></div>";
                    break;
                case "DiscountVal":
                    return "<div class=\"span3 padding-10\"><div class=\"txt\"><span class=\"eBookCheck\">" + parentValue + "</span></div><div class=\"amt\"><b class=\"boldcolor\">$</b><span class=\"AvailabilityChecklogin\">" + value + "</span></div></div>";
                    break;
                case "Stockqty":
                    string allowSalestr="<div class=\"addtocartdiv\"><input type='text' autocomplete='off' maxlength='3' value='1' class=\"Qtycart\"><label class=\"control-label Qtylbl\" for='Qty' >Qty</label><input type='button' onclick='GetIndividualISBN(this," + Isbn + ")' class=\"Btnaddtocart\" value='Add to cart'><input type='button' id='Addedtocart' class=\"addedcart-check ico-addedcart\" style='display:none' value='Added'></div>";
                    if(allowsale!="N")
                        return "<div class=\"span3 padding-10\"><div class=\"availablestock\"><span class=\"eBookCheck\">" + parentValue + "</span></div><div class=\"availableamount\"><span class=\"AvailabilityCheckstock\">" + value + "</span></div></div>" + allowSalestr;
                    else
                        return "<div class=\"span3 padding-10\"><div class=\"availablestock\"><span class=\"eBookCheck\">" + parentValue + "</span></div><div class=\"availableamount\"><span class=\"AvailabilityCheckstock\">" + value + "</span></div></div>";
                    break;
                default:
                    break;
            }
        ...
        /// <returns></returns>
        [WebMethod]
        protected string ShowFavourite(string type, string productsk, string favouriteflag, string favid)
        {
            string html = string.Empty;
            //if (visitor.UserID != 0)
            //{
            ...
            {
                case "ShowFavourite":
                    return "<div class=\"favorite\"><span id=" + favid + productsk + "  class=\"HideItems\" onclick='IconSaveFavorite(" + productsk + ",this)'></span><input type='button' class=\"btn btn-onoff\" value='FAVOURITE' onclick='SaveFavorite(" + productsk + ",this)' /><span class=\"favspan HideItems\"> " + favouriteflag + "</span> <span class=\"HideItems\">" + productsk + "</span></div>";
                    break;
                default:
                    break;
            }
        ...
        protected void FacetsRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            try
            {
                ...
                if (dicAttr[AttrTypeSkHdn.Value].Count == 1)
                {
                    ...
                    LogValues(a.ATTRIBUTE_TYPE_VALUE + " --> IS_SELECTED=" + a.IS_SELECTED + " Hdn-->" + AttrTypeSkHdn.Value);
                    ...
                    else
                    {
                        ...
                        {
                            LogValues("Facet bind -->  " + AttrTypeSkHdn.Value + "--->" + dicAttr[AttrTypeSkHdn.Value].Count);
                            FacetValuesRepeater.DataSource = dicAttr[AttrTypeSkHdn.Value];
                            FacetValuesRepeater.DataBind();
                        }
                    }
        ...
        /// <summary>
        /// Discipline Item Databound Repeater event is to bind all the respective subjects.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void DisciplineRptr_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            try
            {
                HiddenField DisciplineSkHdn = (HiddenField)e.Item.FindControl("DisciplineSkHdn");
                Repeater SubjectRptr = (Repeater)e.Item.FindControl("SubjectRptr");
                SubjectRptr.DataSource = disciplineAttr[DisciplineSkHdn.Value];
                SubjectRptr.DataBind();
            }
            catch (Exception ex)
            {
                LogFileWrite(ex);
        ...
        private string GetDictionary()
        {
            ...
            if (Request.QueryString["division"] == "higher")
            {
                GetDicQuery = "SELECT SEARCH_WORD FROM CEN_HE_DICTIONARY";
                if (Cache["DICT_HE_WORDS"] == null)
                {
                    ...
                    if (DataReader != null)
                        DataReader = null;
                    Cache.Insert("DICT_HE_WORDS", DictionaryWords, null, DateTime.Now.AddHours(1), TimeSpan.Zero);
                    //DnnLog.Trace("Succesfully Loaded Dictionary..");
                }
                else
                {
                    DictionaryWords = ((StringBuilder)Cache["DICT_HE_WORDS"]);
                }
            }
            else
            {
                GetDicQuery = "SELECT SEARCH_WORD FROM CEN_VPG_DICTIONARY";
                if (Cache["DICT_VPG_WORDS"] == null)
                {
                    //DnnLog.Trace("Connecting DB to load dictionary..");
                    IDataReader DataReader = SqlHelper.ExecuteReader(ConfigurationManager.ConnectionStrings["SiteSqlServer"].ConnectionString, CommandType.Text, GetDicQuery);
                    while (DataReader.Read())
                    ...
                    if (DataReader != null)
                        DataReader = null;
                    Cache.Insert("DICT_VPG_WORDS", DictionaryWords, null, DateTime.Now.AddHours(1), TimeSpan.Zero);
                    //DnnLog.Trace("Succesfully Loaded Dictionary..");
                }
                else
                {
                    DictionaryWords = ((StringBuilder)Cache["DICT_VPG_WORDS"]);
                }
            }
